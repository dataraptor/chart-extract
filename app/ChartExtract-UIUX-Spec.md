...
> The UI/UX design specification for the ChartExtract demo: a single-page, web-native interface that turns one messy clinical document into strict, cited, schema-validated JSON.

---

## Step 0. Grounding

- **What it does / who it's for.** ChartExtract turns one messy clinical document (pathology report, intake form, discharge summary) into **strict, schema-validated JSON where every field carries the verbatim source span it came from**. It refuses to invent: a value it can't ground is returned `null` and flagged, never guessed. It ships with a field-level eval harness scoring precision/recall/F1 plus hallucination-rate against a frozen gold set. Personas: **Ops/clinical analyst** (primary, uploads docs, reads structured output); **Engineer/buyer** (meta-user, wants proof the fields are real and the tool knows what it doesn't know, lives in the eval report and span view); **demo audience** (wants one unforgettable visual).

- **THE MONEY DEMO (design everything around this).** Upload a synthetic pathology report, the filled schema appears, then **hover a field and its exact source span lights up in the rendered document.** One field the document doesn't contain returns **`null`, flagged `not_found`**, not hallucinated. A second empty field returns **`null`, flagged `not_assessed`, citing the document's own "Margins not assessed" sentence.** Then the **eval leaderboard**: field-level P/R/F1 on the frozen gold set with **0 hallucinated values.** The pitch is the combination: *cited, honest about gaps (and which kind of gap), and measured.*
...
- **Real data objects the UI must render (actual field names, not invented).**
  ...
  - Eval: per-field **precision / recall / F1**, **macro-F1**, **hallucination-rate** (predicted-non-null where gold is null), **routing accuracy**, reported distributionally (mean ± spread over N runs); GPT-5.5 headline row plus estimated cost-comparison rows.

- **The actual flow, with the states it calls out.** Pick a sample / upload (text-layer PDF or text), then **doc-type routing** (the GPT-5.5 classifier auto-sets a dropdown the human can override; on classifier low-confidence it returns `unknown` and the UI *asks*, never silently guesses), then **extract** (GPT-5.5, structured output, streaming), then **ground in code** (locate each span, producing offsets plus `match_quality`; the model never returns offsets), then **assemble result**. The states the design must demand: streaming/loading; populated; the three empties (`not_found` / `not_assessed` / `not_grounded`); `needs_review` and `ambiguous_span` (kept but uncertain); **no-text-layer doc means highlight disabled with an explicit banner** (vision fallback, offsets unavailable); `refusal` stop_reason and `max_tokens` truncation surfaced, not crashed; doc-type misroute (most fields `not_found`) as a visible, correctable signal.

- **Design decisions.**
  1. `pr_status` renders as a `Field[str]` enum mirroring `er_status`, because the worked example references "PR positive (40%)" as canonical demo content.
  2. The layout keeps the document-left / schema-right spine, since that is the right information architecture for hover-to-highlight, and elevates everything else.
  3. The eval leaderboard is a peer panel within the one screen (a switchable third region), not a second page, because the money demo needs extraction *and* leaderboard legible in one continuous scroll/view.

---

## 1. Design thesis

**ChartExtract should feel like a forensic instrument, not a chatbot.** The single idea the UI is built around: **every claim is wired back to its evidence, and every gap is named.** The interface is a two-sided ledger, the *document* on one side and the *extracted record* on the other, joined by a single physical gesture (hover a field, the source lights up in the text). Trust is not asserted in copy; it is demonstrated by the literal, animated line of sight between a value and the sentence that proves it. Where evidence is missing, the UI says *which kind of missing* in a calm, color-coded, clinical register. The feeling is a senior engineer's tool: quiet, precise, fast, and constitutionally unwilling to overstate what it knows.
...
6. **Fluid, content-driven calm.** Layout breathes as fields stream in; motion is continuous and physical (spans slide into view, panels resize fluidly), never decorative. Density scales with viewport via `clamp()`, not hard breakpoints, so the tool feels native at any width.
...
- `bg` (app background): `#0B0E14`, near-black with a faint blue cast.
...
- **Light mode** (offered as a toggle, since stakeholders screenshot in both): `bg #F7F8FA`, `surface #FFFFFF`, `border #E4E7EC`, `text-primary #11151D`, `text-secondary #4B5566`. All semantic colors below are tuned to hold **WCAG AA (≥4.5:1)** on both `surface` values; the dark-mode hexes below are the canonical set, with light-mode equivalents one step darker in lightness.

**Primary / brand accent** (interactive, non-semantic: buttons, active tab, focus ring)

- `primary`: `#5B8DEF` (a precise, un-flashy blue, distinct in hue from the semantic blue used for `not_assessed`, see note). `primary-hover`: `#6F9CF2`. Focus ring: `primary` at 60% plus 2px offset.
- ⚠ **Hue separation:** brand blue and `not_assessed` blue must read differently. Decision: brand is blue-violet `#5B8DEF`; `not_assessed` is cyan-teal `#3BA7C2`. Different enough at a glance; both AA.
...
| `ambiguous_span` | **amber (hatched)** | `#E2A33C` plus diagonal hatch | grounded, span matched >1 place |
| `not_assessed` | **cyan-teal** | `#3BA7C2` | document *states* it wasn't determined, a cited absence |
| `not_found` | **neutral-grey** | `#6B7484` | document is silent, an honest blank |
| `not_grounded` | **red** | `#E5615E` | model proposed, code couldn't locate it, forced to null (a caught error) |

- **Highlight wash** (the span lit in the document): the field's own flag color at **18% alpha fill plus 100% left-edge bar (3px)**. Accepted fields wash green; a `not_assessed` field washes cyan over the cited "not assessed" sentence; `not_found` has nothing to wash (handled in §6).
...
- **Confidence ramp** (the structural-confidence meter): a 0 to 1 ramp from `#6B7484` (low) to `#E2A33C` (mid, ~τ=0.6) to `#3FB67E` (high), with the **τ threshold marked** as a tick so "below the line means review" is literal.

### Typography

- **Font intent.** Two families, no more. **UI / prose:** a neutral, engineered grotesque (*Inter Tight* or *Söhne*); pick one with true tabular figures (counts, $, latency, offsets must align). **Document body plus all spans/offsets/JSON:** a monospace (*Berkeley Mono* / *JetBrains Mono*), because the rendered clinical source and the verbatim spans should read as *evidence/code*, reinforcing "this is the literal text, character-for-character." The mono in the source pane is the single strongest signal that spans are exact.
...
- **Elevation:** flat by default (borders do the separating). Only two raised levels: popovers/tooltips (`0 4px 16px rgba(0,0,0,.45)` dark) and the field row on hover (a `1px border-strong` plus `surface-raised` fill, *no* shadow, which keeps it crisp).
...
- **What animates and why:**
  - *Hover-to-highlight (the money gesture):* on field hover, the document pane smoothly scrolls the span into view (if off-screen) and the wash fades and expands from the span's left edge over `420ms`; a faint connector pulse runs from field to span. Reverse on hover-out.
  - *Streaming extraction:* field rows fade-up and the right panel grows fluidly as each `GroundedField` arrives, so the layout literally breathes as content lands.
...
- **Reduced motion:** `prefers-reduced-motion` makes all of the above instant opacity swaps; the highlight still appears (just no scroll-glide or glow), and the count-up shows final numbers immediately.

---

## 4. Information architecture

One screen, three regions on a persistent **command bar**. The extraction workspace and the eval leaderboard are **peer views toggled in the same canvas** (not separate pages) so the demo flows from extraction to proof in one continuous motion.
...
│                                                                            │
├──────────────────────────────────────────────────────────────────────────┤
│  RESULT FOOTER  n_grounded 6 · null 2 · review 1 │ GPT-5.5 │ $0.0105 │ 3.4s│
└──────────────────────────────────────────────────────────────────────────┘
```
...
- **Eval Leaderboard view** replaces the two panes with the metrics dashboard, keeping command bar plus footer.
- **Result footer (bottom, sticky):** the `ExtractionResult` counts plus cost, latency, and versions, the honesty meter, always visible.
- **Relationship:** Document and Extraction Panel share one coordinate system (the canonical text plus char offsets). The footer aggregates the panel. The leaderboard is the same product viewed at the population level instead of the single-document level.
...
- **Purpose & data.** Choose input and schema; switch views; keep provenance/disclaimer present. Shows `doc_type` (routed schema key), routing confidence (from the GPT-5.5 classifier), and `model`.
- **Layout.**
...
- **States.**
  ...
  - *Populated, field hovered:* the linked span gets the flag-colored wash plus 3px left bar; if off-screen, the pane glides it into view (`420ms`). Multiple spans can be pinned (see 5.3 click-to-pin).
...
- **Interactions & transitions.** Hover field, get wash plus glide (220 to 420ms). Click span, it stays pinned and the matching field row highlights reciprocally (bidirectional). Selecting text in the document does nothing destructive (read-only evidence).

### 5.3 Extraction panel (right)

- **Purpose & data.** The filled schema as a list of `GroundedField` rows; the structured heart of the output. Each row shows `name`, `value`, the flag (color bar plus chip), structural `confidence` (meter with τ tick), `match_quality`, and the `source_span` snippet.
...
- **States (per row).**
  ...
  - *`not_grounded` (red):* value `null`, red bar, "model proposed a value; span couldn't be located, so it was rejected", explicitly framed as the **safety net working**, with a small "what the model said" disclosure to show the rejected value (the seam from principle 4).
...
- **`ListField` rendering (intake/discharge):** a collapsible group header `medications · 3 of 5 grounded · 2 review`, then per-item rows `medications[0] … medications[1] …`, each independently flagged and hoverable, which proves per-item grounding isn't flattened.
...
- **Interactions & transitions.** Hover row, drive doc wash (5.2). Click row, pin it (both row and span stay lit; multiple pins allowed for comparison). "Show review queue" reorders fluidly to float amber/red/ambiguous to the top (FLIP animation, 220ms). Confidence meters animate to value on stream-in.
...
```
6 grounded · 2 null (1 not_found · 1 not_assessed) · 1 review   |   GPT-5.5   |   $0.0105/run   |   3.4s   |   prompt v3 · schema v2
```

- **States.** Loading: counts tick up live as fields ground; cost/latency fill on completion. Error: still shows model plus version so the failure is attributable.
- **Interactions.** Clicking the counts scrolls/filters the panel to that subset (e.g. click "1 review" to open the review queue). The `$0.0105` has a tooltip breaking down input/output/cache tokens (ties to §10 caching honesty).

### 5.5 Eval leaderboard view

- **Purpose & data.** The headline artifact: per-field **precision/recall/F1**, **macro-F1**, **hallucination-rate**, **routing accuracy**, over the frozen gold set, distributionally (mean ± spread, N runs), with the **statistics-honesty caption** (small gold set means wide intervals).
- **Layout (ASCII).**

```
┌ EVAL · frozen gold set · 17 docs · N=1 run ──────────────────────────┐
│                                                                       │
...
│   …                                                                   │
│                                                                       │
│   Routing accuracy  0.95   │   Model: GPT-5.5                         │
│   ── cost comparison (estimated) ──                                   │
│   Opus 4.8    F1 0.94   $0.0105/doc                                   │
│   Sonnet 4.6  F1 0.90   $0.0063/doc  ↓40% vs Opus                    │
│                                                                       │
│   ⓘ 17 synthetic docs, report F1 with wide intervals; not real PHI.  │
└───────────────────────────────────────────────────────────────────────┘
```

- **States.** Empty (no run yet): "Run the frozen eval" CTA. Running: per-field bars fill, count-up. Populated: the **hallucination-rate "0" lands last with a green ring** (the climax). Error/partial run: shows completed docs plus a "k of n scored" note, never a fake total.
- **Interactions & transitions.** Numbers count up on enter; clicking a field row could (stretch) drill into the per-doc breakdown. The estimated Opus/Sonnet cost-comparison rows let the buyer see the **honest cost/accuracy trade**.
...
2. **Beat 1, stream-in (0.0 to 3.4s).** Fields land top-to-bottom, 40ms stagger, each fading up with its confidence meter filling and green bars appearing: `specimen`, `diagnosis`, `grade`, `tumor_size_cm`, `er_status`, `pr_status`, `her2_status`. The panel grows fluidly; footer counts tick up. This establishes the happy path *fast*.
...
6. **Beat 5, (optional) the safety net (`not_grounded`).** A toggle ("show a caught hallucination") swaps in a variant where the model proposed a value with an unlocatable span, producing a red row, "model proposed; span couldn't be located, so it was rejected." Reinforces that grounding is enforced in code, not promised.
...
- **SourceSelector** sample dropdown plus upload drop-zone; states: empty, dragover, uploading, loaded, unsupported-file.
- **SchemaDropdown** auto / manual / unknown(amber, required); shows `doc_type` plus routing confidence.
- **DocumentPane** states: empty, streaming, idle, hover-active, ambiguous-marked, no-text-layer-banner, refusal-notice.
- **SpanHighlight** the wash plus left bar; variants by flag color; sub-states: single, pinned, ambiguous (dotted plus "1 of N"), off-screen-glide.
...
- **FlagChip** icon plus label per taxonomy; tooltip with the one-line meaning.
- **ConfidenceMeter** 0 to 1 ramp with τ tick; structural (not model) value; animates to position.
- **ListFieldGroup** collapsible header ("3 of 5 grounded · 2 review") plus per-item FieldRows (`name[i]`).
- **JSONInspector** collapsible syntax-highlighted `ExtractionResult`; the offsets/match_quality verifier.
- **ResultFooter** counts plus model, cost, latency, versions; clickable to filter.
- **EvalLeaderboard** MetricHeadline (macro-F1, hallucination-rate-with-ring), FieldMetricRow (P/R/F1 plus distribution bar), RoutingAccuracy, CostComparisonRow (estimated Opus vs Sonnet), StatsHonestyCaption.
...
## 8. Responsive & fluid behavior (wide to narrow)
...
- **Medium (~900 to 1280px):** panes stay side-by-side but gutters compress; confidence meters collapse to a compact dot plus number; JSON inspector defaults closed.
- **Narrow / stacked (<900px):** panes stack, with the Extraction panel on top (the answer) and the Document below (the evidence). Hover becomes **tap-to-link**: tapping a field smooth-scrolls to its span and washes it; a floating "back to fields" pill returns. Footer wraps to two lines; leaderboard table becomes stacked metric cards.
- **The hover gesture degrades gracefully:** pointer becomes hover; touch becomes tap-to-pin-and-scroll; keyboard means focus drives the same wash (see §9). The bidirectional link is preserved at every size.
...
- **Contrast:** all text and semantic colors meet **WCAG AA (≥4.5:1)** on both dark and light surfaces; flag colors are never the *only* signal, each pairs with an icon plus text label (color-blind safe; `ambiguous_span` adds a hatch pattern, `not_grounded` adds a distinct icon).
- **Keyboard paths:** full tab order, command bar to field rows to JSON to footer to view toggle. Arrow keys move between FieldRows; focusing a row triggers the same span wash plus doc glide as hover; `Enter` pins; `Esc` clears pins. The leaderboard is fully tabbable. No keyboard trap in the upload zone.
...
- **Error/edge (all surfaced, none crash):** no-text-layer becomes a highlight-disabled banner; `max_tokens` truncation becomes an auto-retry notice; `refusal` becomes a calm skip notice; doc-type misroute becomes most fields grey `not_found` plus a "looks misrouted, change schema?" hint; unsupported upload becomes inline "text-layer PDF or .txt only (OCR out of scope)."
...
1. **`01-extraction-hero-hover.png`** The money frame. Pathology doc left (mono, "Tumor size 1.4 cm" washed **green**), Extraction panel right with all rows populated, `tumor_size_cm` row hovered, connector pulse, footer counts plus $0.0105 plus 3.4s. Dark mode. *This is the hero screenshot.*
2. **`02-two-honest-nulls.png`** Dual focus on `margin_status` (cyan `not_assessed`, "Margins not assessed" washed cyan in doc) and `lymph_nodes_positive` (grey `not_found`, nothing washed). The named-gaps story in one image.
...
4. **`04-eval-leaderboard.png`** Full leaderboard: macro-F1 0.94, **hallucination-rate 0 with green ring**, per-field P/R/F1 distribution bars, routing accuracy 0.95, estimated Opus-vs-Sonnet cost row, stats-honesty caption.
5. **`05-streaming-state.png`** Mid-extraction: 4 rows landed (green, confidence meters filling), 3 still skeleton/shimmer, panel mid-growth, capturing the "layout breathes as content streams" fluidity.
6. **`06-listfield-intake.png`** Intake schema with a `ListField`: `medications · 3 of 5 grounded · 2 review` collapsible group, per-item rows with mixed flags, which proves per-item grounding plus routing beyond pathology.
...
8. **`08-narrow-stacked.png`** Mobile/narrow: Extraction panel stacked above Document, a field tapped so its span is scrolled-to and washed, "back to fields" pill. Shows fluid responsive behavior. (Optionally render light mode here to show both themes.)
