...
Open <http://127.0.0.1:8000/>. The `app/ChartExtract.dc.html` UI is served from the same origin, so
no CORS is needed. With `AZURE_OPENAI_API_KEY` set the server runs the live GPT-5.5 provider;
...
|--------|-----------------|------------------------------------------------------------|---------|
| GET    | `/health`       | (none)                                                     | `{status, provider, model}` |
| GET    | `/api/samples`  | (none)                                                     | `[{id, name, doc_type_hint, text}]` (text is the canonical offset source) |
| POST   | `/api/extract`  | JSON `{sample_id?, text?, schema?, provider?}` or multipart `{file, schema?, provider?}` | `ExtractionResult` (200) |
| GET    | `/api/eval`     | (none)                                                     | Eval leaderboard summary |

Exactly one of `sample_id` / `text` / uploaded `file` is required. `schema` (`pathology` or
`intake`) overrides routing; omit it to route via the classifier. `provider` is `stub` or `openai`.
